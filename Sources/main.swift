...
final class StatusController: NSObject, NSMenuDelegate {
    ...
    struct Session {
        ...
        init(json o: [String: Any], id: String) {
            ...
            self.sessOut = (o["sessOut"] as? NSNumber)?.intValue ?? 0
    ...
    func ensureHooksInstalled() {
        ...
        DispatchQueue.global().async {
            guard let node = Self.locateNode() else {
                NSLog("ClaudeBeacon: could not find node; hooks not installed (will retry next launch)")
                return
            }
            let task = Process()
    ...
    let releaseAPIURL = "https://api.github.com/repos/Gikunjuu/claude-beacon/releases/latest"
    let releasePageURL = "https://github.com/Gikunjuu/claude-beacon/releases/latest"
    ...
    func checkForUpdate() {
        ...
        req.setValue("ClaudeBeacon", forHTTPHeaderField: "User-Agent") // GitHub API requires a UA
